              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 flex-1 min-h-0">
                  ...
                  <div className="flex flex-col gap-4 min-h-0">
                    ...
                    <div className="card p-4">
                      ...
                      {territory.storedGp != null && (
                        <div className="mt-3 pt-3 border-t border-[#1e2a3d]">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-muted text-[10px]">Storage GP</p>
                              <p className="font-bold text-[13px]" style={{ color: '#00ff88' }}>
                                🏦 {(storedGpLocal ?? territory.storedGp).toLocaleString()}
                              ...
                              {territory.productionRatePerMin != null && (
                                <p className="text-muted text-[9px]">+{territory.productionRatePerMin}/분 생산 중</p>
                              )}
                              {collectMsg && (
                                <p className="text-[10px]" style={{ color: collectMsg.includes('실패') ? '#ff5555' : '#00ff88' }}>{collectMsg}</p>
                              )}
                            </div>
                            {isMyTerritory && (
                              <button onClick={() => void handleCollectStorage()} disabled={isCollectingStorage}
                    ...
                    {/* Siege declaration */}
                    {!isMyTerritory && territory.status === 'OCCUPIED' && isLoggedIn && (
                      <div className="card p-4">
                        <p className="text-foreground font-semibold mb-3 text-xs">⚔ 공성전 선언</p>
                        {siegeMessage && (
                          <p className="text-xs mb-2 px-2 py-1.5 rounded-lg" style={{ background: '#00ff8820', color: '#00ff88', border: '1px solid #00ff8840' }}>{siegeMessage}</p>
                        )}
                        {siegeError && (
                          <p className="text-xs mb-2 px-2 py-1.5 rounded-lg" style={{ background: '#ff333320', color: '#ff3333', border: '1px solid #ff333340' }}>{siegeError}</p>
                        )}
                        <div className="grid grid-cols-2 gap-2 mb-3">
                          <div>
                            <p className="text-muted mb-1 text-[10px]">공격 유형</p>
                            <select value={attackType} onChange={e => setAttackType(e.target.value as 'NORMAL' | 'PRECISION')}
                          ...
                          <div>
                            <p className="text-muted mb-1 text-[10px]">공격 구역</p>
                            <select value={attackZone} onChange={e => setAttackZone(Number(e.target.value))}
                              className="w-full px-2 py-1.5 rounded-lg text-[11px]"
                              style={{ background: '#2a3050', color: '#e0e8ff', border: '1px solid #354064' }}>
                              <option value={1}>1구역 (코어)</option>
                              <option value={2}>2구역</option>
                              <option value={3}>3구역 (외곽)</option>
                            </select>
                          </div>
                        </div>
                        {siegeUnits.length > 0 && (
                          <div className="space-y-1.5 mb-3">
                            <p className="text-muted mb-1 text-[10px]">투입 유닛</p>
                            {siegeUnits.map(u => (
                              <div key={u.unitTypeId} className="flex items-center gap-2">
                                <span className="flex-1 text-[11px] text-foreground">{u.name} (대기 {u.idleCount})</span>
                                <input type="number" min={0} max={u.idleCount}
                                  value={siegeUnitQtys[u.unitTypeId] ?? 0}
                                  onChange={e => setSiegeUnitQtys(prev => ({ ...prev, [u.unitTypeId]: Number(e.target.value) }))}
                                  className="w-14 rounded px-1 py-0.5 text-[11px] text-center"
                                  style={{ background: '#2a3050', color: '#e0e8ff', border: '1px solid #354064' }} />
                              </div>
                            ))}
                          </div>
                        )}
                        <button onClick={() => void handleDeclareSiege()} disabled={isSieging}
                          className="w-full h-9 rounded-xl text-xs font-bold disabled:opacity-50 transition-all hover:brightness-110"
                          style={{ background: '#ff3333', color: '#fff' }}>
                          {isSieging ? '선언 중...' : '⚔ 공성전 선언'}
                        </button>
                      </div>
                    )}
