...
public class UnitTypeSeeder implements ApplicationRunner {
    ...
    // 관리자가 수치를 편집할 수 있으므로 yml은 초기값 역할만 한다 — 없는 유닛만 새로 만든다.
    @Override
    @Transactional
    ...
    private void createIfAbsent(Map<String, Object> row) {
        ...
        if (unitTypeRepository.findByName(name).isPresent()) return;
        ...
        specs.forEach(spec -> unitTypeLevelSpecRepository.save(toSpec(unitType, spec)));
    }
    ...
    private List<Map<String, Object>> loadEntries() {
        Yaml yaml = new Yaml();
        try (InputStream is = getClass().getResourceAsStream("/db/unit-types.yml")) {
            Map<String, Object> root = yaml.load(is);
            return (List<Map<String, Object>>) root.get("unitTypes");
        } catch (Exception e) {
            throw new IllegalStateException("unit-types.yml 로드 실패", e);
        }
    }
}
